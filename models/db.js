...
async function DeleteDocument(model, criteria, callback) {
  try {
    const result = await DB[model].deleteOne(criteria);
    callback(null, result);
  } catch (err) {
    callback(err, null);
  }
...
async function UpdateDocument(model, criteria, doc, options, callback) {
  try {
    const result = await DB[model].update(criteria, doc, options);
    callback(null, result);
  } catch (err) {
    callback(err, null);
  }
